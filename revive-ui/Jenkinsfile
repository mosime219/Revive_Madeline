pipeline {
    ...
    // environment {
    //     SONAR_TOKEN = credentials('sonar') // Sonar token
    // }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'ui', url: 'git@github.com:mosime219/Revive_Madeline.git', credentialsId: 'ssh-Agent'
            }
        }
        ...
        //                     -Dsonar.projectKey=UI-microserve \
        //                     -Dsonar.host.url=http://54.82.200.211:9000/ \
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    ...
                    sh '''
                        cd revive-ui/ui
                        docker build -t mosime/revive:ui .
                    '''
                }
            }
        }
